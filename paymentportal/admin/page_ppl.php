...
if( isset($_POST['ConfirmDelete']) )
{
	...
	else
	{
		...
		// delete peoples
		$updateSQL = sprintf("DELETE FROM people WHERE page_id = %s", $page_id );
...
mysql_select_db($db_database, $db_connection);
$query_rsPeople = sprintf("SELECT * FROM people WHERE page_id = %s ORDER BY sort_by, last_name, first_name", $page_id);
//echo $query_rsPeople;
$rsPeople = mysql_query($query_rsPeople, $db_connection) or die(mysql_error());
$row_rsPeople = mysql_fetch_assoc($rsPeople);
$varNumPeople = mysql_num_rows($rsPeople);
?>
  ...
  <?php if( !$deleting ) { ?>
      <tr>
      	<td colspan="4">
            ...
            <tr>
            <td>
          <form id="form0" name="form0" method="post" action="page_ppl_add.php">
      ...
      <td class="tableHeadings" width="100">Display Order</td>
	  	...
	  	if( $varNumPeople > 0 ) 
		{
         ...
         <tr>
            ...
            <form id="form<?php echo $row_rsPeople['people_id']; ?>" name="form<?php echo $row_rsPeople['people_id']; ?>" method="post" action="page_ppl_edit.php">
        <input type="hidden" name="page_id" value="<?php echo $page_id; ?>" />
        <input type="hidden" name="page_type_id" value="<?php echo $page_type_id; ?>" />
        <input type="hidden" name="page_title" value="<?php echo $page_title; ?>" />
         <input type="hidden" name="people_id" value="<?php echo $row_rsPeople['people_id']; ?>">
      	<td  class="tableFields" ><?php echo $row_rsPeople['sort_by']; ?>        </td>
        <td  class="tableFields" >
         
         <?php echo $row_rsPeople['first_name'] . ' ' . $row_rsPeople['last_name']; ?>         </td>
        <td  class="tableFields" >
        <?php echo $row_rsPeople['title']; ?> 
       </td>
		<?php if( $page_id == 3 ) { ?>
        <td  class="tableFields" >
        <?php echo $row_rsPeople['desc_2']; ?> 
       </td>
        <td  class="tableFields" >
        <?php if( $row_rsPeople['desc_3'] > 0 ) echo getCampusName( $row_rsPeople['desc_3'], $db_database, $db_connection); ?> 
       </td>
		<?php } ?>  
        <td  class="tableFields" >
        <?php echo ($row_rsPeople['active'] == "on" ? "Active" : "Inactive"); ?> 
       </td>             
    ...
    <?php  
           } while ($row_rsPeople = mysql_fetch_assoc($rsPeople));
        }  
  ?>    
      </table>
  <br/>
