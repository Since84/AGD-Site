...
$returnURL = "page_edit_pdfs.php?page_id=" . $page_id . '&page_title=' . $page_title;
...
if( isset($_POST["ADDPDF"]) )
{
	...
	if($edits_ok)
	{
		...
		{
			$msg .= '<font color="red">Please select PDF to add.</font><br/>';
		}
		...
		if( $upload_ok )
		{
			...
			// now update database
			require('pdfs_add.php'); 
			$product_id = addPDF($db_filename_pdf, $db_database, $db_connection);
			header(sprintf("Location: %s", $returnURL));
			exit();
		}
	}	
...
<span class="pagetitle">ADD  to "<?php echo $page_title; ?>"</span>
     ...
     <tr>
	  ...
	  <input type="hidden" name="ADDPDF" id="ADDPDF" value="1" />
	  ...
	  <tr>
	  	<td valign="top">&nbsp;</td>
		<td colspan="3"> 
        <input name="ADDPDF" type="submit" id="ADDPDF" value="Add" />
