...
if( isset($_FILES['image3_file']['name']) && strlen($_FILES['image3_file']['name']) > 1 )
{
	...
	$checks_ok = true;
	if( $fsize_photo >  $imageDef->getMaxImageSize() || $_FILES['image3_file']['error'] == 2 )
	{
			$msg .= '<p><font color="red">The image file (' . $_FILES['image3_file']['name'] . ') size exceeds maximum file size of ' . $imageDef->getMaxImageSize() . ' KB.</font></p>';
		$checks_ok = false;
	}
	else
	{
		...
		else
		{
			...
			$filename = $imageDef->getPrefix() . '3_' . $_POST['name'] . '_' . $date_time . '.' . $extension[1];
			...
			$full_image3_filename 	= "../" . $imageDef->getImageFolder() . $filename;
			$full_fullsize_filename = "../" . $imageDef->getImageFolder() . 'fullsize/' . $filename;

			if( $TESTING )
			{
				echo '<br/>full_image3_filename: ' . $image3_filename . ', ' . $full_image3_filename;
			}

			// check if file already exists
			if( file_exists($full_thumb_filename) )
				$do = unlink($full_thumb_filename);
			if( file_exists($full_image3_filename) )
				$do = unlink($full_image3_filename);
			...
			if($extension[1] == 'png' && function_exists('imagecreatefrompng') || $extension[1] == 'jpg' && function_exists('imagecreatefromjpeg') || $extension[1] == 'jpeg' && function_exists('imagecreatefromjpeg') || $extension[1] == 'gif' && function_exists('imagecreatefromgif')  || $extension[1] == 'bmp' && function_exists('imagecreatefromwbmp')) 
			{
				...
				if($extension[1] == 'jpg' || $extension[1] == 'jpeg')
				{
					...
					imageJPEG($image, $full_image3_filename, 80);
				...
				elseif($extension[1] == 'png')
				{
					...
					imagePNG($image, $full_image3_filename);
				...
				elseif($extension[1] == 'gif')
				{
					...
					imageGIF($image, $full_image3_filename);
