  ...
  <table width="80%" >
      <tr>
      	<td colspan="6">
            ...
            <tr>
            <td>
          <form id="form0" name="form0" method="post" action="page_help_add.php">
            <input type="submit" name="add" id="add" value="Add Help" />
            <input type="hidden" name="page_id" value="<?php echo $page_id; ?>" />
            <input type="hidden" name="page_title" value="<?php echo $page_title; ?>" />
         </form>   
           <td>
      ...
      <tr>
      	...
      	<td class="tableHeadings"> Help Link  </td>
      	<td class="tableHeadings"> Help File  </td>
	  	...
	  	if( $varNumResources > 0 ) 
		{
         ...
         <tr>
            ...
            <form id="form<?php echo $row_rsHelp['help_id']; ?>" name="form<?php echo $row_rsHelp['help_id']; ?>" method="post" action="page_help_edit.php">
        <input type="hidden" name="page_id" value="<?php echo $page_id; ?>" />
        <input type="hidden" name="page_title" value="<?php echo $page_title; ?>" />
         <input type="hidden" name="help_id" value="<?php echo $row_rsHelp['help_id']; ?>">
        <td  class="tableFields"  >
        <?php echo getHelpCatDesc($row_rsHelp['help_cat_id'], $db_database, $db_connection); ?> 
       </td>
        <td  class="tableFields" >
        <?php echo $row_rsHelp['sort_by']; ?> 
       </td>
        <td  class="tableFields" >
        <?php echo $row_rsHelp['title']; ?> 
       </td>
        <td  class="tableFields" >
        <?php if( strlen($row_rsHelp['resource_link']) > 0 ) { ?>
        <a href="<?php echo $row_rsHelp['resource_link']; ?>" target="_blank">View Link</a>
        <?php } ?>
       </td>
        <td  class="tableFields" >
        <?php if( strlen($row_rsHelp['resource_file']) > 0 ) { ?>
        <a href="../<?php echo $HELP_FOLDER; ?><?php echo $row_rsHelp['resource_file']; ?>" target="_blank">View File</a>
        <?php } ?>
       </td>
    ...
    <?php  
           } while ($row_rsHelp = mysql_fetch_assoc($rsHelp));
        }  
  ?>    
      </table>
  <br/>
