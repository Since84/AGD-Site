...
$query_rsVideos = sprintf("SELECT * FROM videos WHERE page_id = %s AND swf != '' ORDER BY display_order, title ", $page_id);
//echo $query_rsVideos;
...
<span class="pagetitle"> <?php echo $page_title; ?>  </span>
    ...
    <?php if( $just_videos ) { ?>
    <form id="form0" name="form0" method="post" action="<?php echo $returnURL; ?>">
     <input type="hidden" name="page_id" value="<?php echo $page_id; ?>">
      <input type="hidden" name="page_title" value="<?php echo $page_title; ?>" />
  ...
  <table width="800" >
      <tr>
      	<td colspan="4">
            ...
            <tr>
            <td>
          <form id="form0" name="form0" method="post" action="page_videos_add.php">
            <input type="submit" name="add" id="add" value="Add Video" />
            <input type="hidden" name="page_id" value="<?php echo $page_id; ?>" />
            <input type="hidden" name="page_title" value="<?php echo $page_title; ?>" />
            <input type="hidden" name="just_videos" value="<?php echo $just_videos; ?>" />
         </form> 
         </td>
         <td> 
      ...
      <td class="tableHeadings" width="100">Display Order</td>
	  	...
	  	if( $varNumVideos > 0 ) 
		{
         ...
         <tr>
            ...
            <form id="form<?php echo $row_rsVideos['video_id']; ?>" name="form<?php echo $row_rsVideos['video_id']; ?>" method="post" action="page_videos_edit.php">
        <input type="hidden" name="page_id" value="<?php echo $page_id; ?>" />
        <input type="hidden" name="page_title" value="<?php echo $page_title; ?>" />
         <input type="hidden" name="video_id" value="<?php echo $row_rsVideos['video_id']; ?>">
         <input type="hidden" name="just_videos" value="<?php echo $just_videos; ?>" />

      	<td  class="tableFields" ><?php echo $row_rsVideos['display_order']; ?>        </td>
        <td  class="tableFields" >
         
         <?php echo stripslashes($row_rsVideos['title']); ?>         </td>
