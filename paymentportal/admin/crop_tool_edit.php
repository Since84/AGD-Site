...
if( isset($_POST['image_lib_id']) )
{
	$image_lib_id		= $_POST['image_lib_id'];
}
else
{
	$image_lib_id		= $_GET['image_lib_id'];
}
...
$returnURL = "crop_tool.php";

$_SESSION['returnURL'] = sprintf("%s?image_lib_id=%s", "crop_tool_edit.php", $image_lib_id);
...
if( isset($_POST["MM_update"]) )
{
	...
	{
		$gotoURL = 'photoadd.php?id=' . $image_lib_id . '&type_id=' . $page_type_id . '&title=' . $title;
		header(sprintf("Location: %s", $gotoURL));
		exit();
	}
	if( isset($_POST['ImageReplace'])  )
	{
		$gotoURL = 'photoreplace.php?id=' . $image_lib_id . '&type_id=' . $page_type_id . '&title=' . $title;
		header(sprintf("Location: %s", $gotoURL));
		exit();
	}
	if( isset($_POST['ImageRemove'])  )
	{
		$gotoURL = 'photoremove.php?id=' . $image_lib_id . '&type_id=' . $page_type_id . '&title=' . $title;
		header(sprintf("Location: %s", $gotoURL));
		exit();
	}
	...
	if( $_POST['delete'] == true  )
	{
			if( $image_file != NULL )
			{
				...
				deletePhotos("cms/lib/", $image_file);
			}
			// delete item
			$updateSQL = sprintf("DELETE FROM image_lib WHERE image_lib_id = %s LIMIT 1", $image_lib_id );
	...
	else
	{
		...
		if($edits_ok)
		{
			require('db_image_lib_update.php'); 
			$result = updateImagelib($db_database, $db_connection);	
...
else
{
	...
	mysql_select_db($db_database, $db_connection);
	$query_rsImageLib = sprintf("SELECT * FROM image_lib WHERE image_lib_id = %s", $image_lib_id);
	if( $DEBUGGING )
		echo $query_rsImageLib;
	$rsImageLib = mysql_query($query_rsImageLib, $db_connection) or die(mysql_error());
	$row_rsImageLib = mysql_fetch_assoc($rsImageLib);

	$sort_by		= $row_rsImageLib['sort_by'];	
	$title 			= $row_rsImageLib['title'];	
	$group_id 		= $row_rsImageLib['group_id'];	
	$image_file		= $row_rsImageLib['image_file'];	
	$short_desc 	= stripslashes($row_rsImageLib['short_desc']);	
	$description 	= stripslashes($row_rsImageLib['description']);	
}
...
<span class="pagetitle"><?php echo ($deleting ? "DELETE" : "EDIT"); ?> from "<?php echo $page_title; ?>"</span>
     ...
     <tr>
      ...
      <input type="hidden" name="image_lib_id" value="<?php echo $image_lib_id; ?>">
