...
if( isset($_FILES['pdf_file2']['name']) && strlen($_FILES['pdf_file2']['name']) > 1 )
{
	...
	{
			$msg .= '<p><font color="red">The pdf file (' . $_FILES['pdf_file2']['name'] . ') size exceeds maximum file size of ' . $max_pdf_size . ' KB.</font></p>';
		$checks_ok = false;
	}
	else
	{
		...
		else
		{
			...
			$db_filename2_pdf = $pdf_prefix . $filename_pdf;
			$full_filename_pdf = "../" . $pdf_folder . '/' . $pdf_prefix . $filename_pdf;
