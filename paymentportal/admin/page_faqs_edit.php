...
$_SESSION['returnURL'] = sprintf("%s?page_id=%s&page_title=%s&faq_id=%s", "page_faqs_edit.php", $page_id, $page_title, $faq_id);
...
if( isset($_POST["MM_update"]) )
{
	...
	{
			// delete item
			$updateSQL = sprintf("DELETE FROM faqs WHERE faq_id = %s LIMIT 1", $faq_id );
	...
	else
	{
		...
		if($edits_ok)
		{
			require('db_faq_update.php'); 
			$result = updateFAQ($db_database, $db_connection);	
...
else
{
	...
	mysql_select_db($db_database, $db_connection);
	$query_rsFAQs = sprintf("SELECT * FROM faqs WHERE faq_id = %s", $faq_id);
	if( $DEBUGGING )
		echo $query_rsFAQs;
	$rsFAQs = mysql_query($query_rsFAQs, $db_connection) or die(mysql_error());
	$row_rsFAQs = mysql_fetch_assoc($rsFAQs);

	$display_order	= $row_rsFAQs['display_order'];	
	$question 		= stripslashes($row_rsFAQs['question']);	
	$answer 		= stripslashes($row_rsFAQs['answer']);		
}
