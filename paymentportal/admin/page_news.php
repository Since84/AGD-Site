...
if( isset($_POST['ConfirmDelete']) )
{
	...
	else
	{
		...
		// delete news items
		$updateSQL = sprintf("DELETE FROM news WHERE page_id = %s", $page_id );
...
mysql_select_db($db_database, $db_connection);
$query_rsNews = sprintf("SELECT * FROM news WHERE page_id = %s %s ORDER BY news_date DESC ", $page_id, $strWHERE);
//echo $query_rsNews;
$rsNews = mysql_query($query_rsNews, $db_connection) or die(mysql_error());
$row_rsNews = mysql_fetch_assoc($rsNews);
$varNumNews = mysql_num_rows($rsNews);
?>
   ...
   <?php if( !$deleting ) { ?>
      <tr>
      	<td colspan="4">
            ...
            <tr>
            <td>
          <form id="form0" name="form0" method="post" action="page_news_add.php">
            <input type="submit" name="add" id="add" value="Add News" />
	        <input type="hidden" name="page_type_id" value="<?php echo $page_type_id; ?>">
            <input type="hidden" name="page_id" value="<?php echo $page_id; ?>" />
         </form> 
         </td>
        ...
        <?php } ?>
      <tr>
      <td class="tableHeadings" width="100">News Date</td>
      	<td class="tableHeadings">Title	</span>        </td>
      	<td class="tableHeadings">Status	</span>        </td>
        ...
        <form id="form<?php echo $row_rsNews['news_id']; ?>" name="form<?php echo $row_rsNews['news_id']; ?>" method="post" action="page_news_edit.php">
        <input type="hidden" name="page_id" value="<?php echo $page_id; ?>" />
        <input type="hidden" name="page_type_id" value="<?php echo $page_type_id; ?>">
        <input type="hidden" name="title" value="<?php echo $title; ?>">
		<input type="hidden" name="news_id" value="<?php echo $row_rsNews['news_id']; ?>">
      	<td  class="tableFields" ><?php echo $row_rsNews['news_date']; ?>        </td>
        <td  class="tableFields" >
         
         <?php echo $row_rsNews['title']; ?>         </td>
        <td  class="tableFields" >
        <?php echo ( $row_rsNews['active'] < 1 ? "Inactive" : "Active"); ?>
       </td> 
