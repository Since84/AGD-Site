...
$strWHERE = " AND archive is NULL ";
if( $archive == 'on' )
	$strWHERE = " AND archive = 'on' ";
...
if( isset($_POST['ConfirmDelete']) )
{
	...
	else
	{
		...
		// delete peoples
		$updateSQL = sprintf("DELETE FROM portfolio WHERE page_id = %s", $page_id );
...
mysql_select_db($db_database, $db_connection);
$query_rsPortfolio = sprintf("SELECT * FROM portfolio WHERE page_id = %s %s ORDER BY group_id, sort_by, title", $page_id, $strWHERE);
  ...
  <?php if( !$deleting ) { ?>
      <tr>
      	<td colspan="4">
            ...
            <tr>
            <td>
          <form id="form0" name="form0" method="post" action="page_portfolio_add.php">
            <input type="submit" name="add" id="add" value="Add a Portfolio" />
            <input type="hidden" name="page_id" value="<?php echo $page_id; ?>" />
            <input type="hidden" name="page_type_id" value="<?php echo $page_type_id; ?>" />
            <input type="hidden" name="page_title" value="<?php echo $page_title; ?>" />
         </form>   
	   ...
	   <option value="" <?php if($archive == "") echo ' selected="selected" '; ?>>Not Archived</option>
	   <option value="on" <?php if($archive == "on") echo ' selected="selected" '; ?>>Archived</option>
       </select>
       <label>
       <input type="submit" name="GetNewList" id="GetNewList" value="Update List" />
       </label>
     </form>
             </td>
         </tr>
      ...
      <tr>
        ...
        <td class="tableHeadings" width="100">Display Order</td>
      	...
      	<td class="tableHeadings"></span>Status       </td>
        <td class="tableHeadings">&nbsp;</td>
      </tr>
  		 <!-- first look for sub categories -->
		 <?php          
	  	if( $varNumPortfolios > 0 ) 
		{
		  	do
			{
		 ?>
         <tr>
            ...
            <form id="form<?php echo $row_rsPortfolio['portfolio_id']; ?>" name="form<?php echo $row_rsPortfolio['portfolio_id']; ?>" method="post" action="page_portfolio_edit.php">
        <input type="hidden" name="page_id" value="<?php echo $page_id; ?>" />
        <input type="hidden" name="page_type_id" value="<?php echo $page_type_id; ?>" />
        <input type="hidden" name="page_title" value="<?php echo $page_title; ?>" />
         <input type="hidden" name="portfolio_id" value="<?php echo $row_rsPortfolio['portfolio_id']; ?>">
      	<td  class="tableFields" ><?php echo $row_rsPortfolio['group_id']; ?>        </td>
        <td  class="tableFields" ><?php echo $row_rsPortfolio['sort_by']; ?>        </td>
        <td  class="tableFields" >        <?php echo $row_rsPortfolio['title']; ?>        </td>
        <td  class="tableFields" >        <?php if( $row_rsPortfolio['archive'] == "on" ) echo "Archived"; else if( $row_rsPortfolio['active'] == "on" ) echo "Active"; else echo 'Inactive'; ?>        </td>
		 <td width="200" class="tableFields" >
         <?php if( !$deleting ) { ?>
          <input name="Update" type="submit" id="Update" value="Edit" />
          <?php if( $archive == "on" ) { ?>
          <input name="UnArchive" type="submit" id="UnArchive" value="UnArchive" />
          <?php } else { ?>
          <input name="Archive" type="submit" id="Archive" value="Archive" />
          <?php } ?>
    ...
    <?php  
           } while ($row_rsPortfolio = mysql_fetch_assoc($rsPortfolio));
        }  
  ?>    
      </table>
  <br/>
