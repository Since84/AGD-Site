...
if( isset($_POST["ADDRESOURCE"]) )
{
	...
	if( !(isset($_FILES['resource_file']['name']) || isset($_POST['resource_link'])) )
	{
		$msg = $msg . '<font color="red">Please enter a resource link or file to upload. ".</font><br/>';
		$edits_ok = false;
	}
	...
	if($edits_ok)
	{
		...
		if( $upload_ok )
		{
			...
			// now update database
			require('db_resources_add.php'); 
			$product_id = addResource($db_filename_pdf, $db_database, $db_connection);
			header(sprintf("Location: %s", $returnURL));
			exit();
		}
		else
